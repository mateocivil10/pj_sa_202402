...
# Función para reemplazar NaN en columnas 'object' con la moda
def aplicar_acciones(df_muestra, acciones_df):
    for _, row in acciones_df.iterrows():
        variable = row['Variable']
        accion = row['Acción']
        
        if variable in df_muestra.columns:  # Verificar si la variable está en el DataFrame
                        
            if accion == 'quitar':
                # Eliminar la columna
                df_muestra.drop(columns=[variable], inplace=True)
            elif accion == 'cero':
                # Reemplazar los NaN con 0
                df_muestra[variable].fillna(0, inplace=True)
            elif accion == 'moda':
                # Si la columna es de tipo object (categórica o de texto), reemplazar NaN con la moda
                if df_muestra[variable].dtype == 'object':
                    moda = df_muestra[variable].mode(dropna=True)[0]  # Obtener la moda, excluyendo NaN
                    df_muestra[variable].fillna(moda, inplace=True)
                else:
                    moda = df_muestra[variable].mode(dropna=True)[0]  # Obtener la moda, excluyendo NaN
                    df_muestra[variable].fillna(moda, inplace=True)
                    #print(f"Advertencia: La columna '{variable}' no es de tipo 'object' para la acción 'moda'.")
            elif accion == 'antigedif':
                # Reemplazar los NaN con el valor de la columna 'antigedif'
                if 'antigedif' in df_muestra.columns:  # Verificar si 'antigedif' existe
                    df_muestra[variable].fillna(df_muestra['antigedif'], inplace=True)
                else:
                    print(f"Advertencia: La columna 'antigedif' no se encuentra en el DataFrame.")
        else:
            print(f"Advertencia: La columna '{variable}' no se encuentra en el DataFrame.")
    
    return df_muestra

    

def generar_resumen_nan(df_muestra):
    # Ajustar la visualización para mostrar más filas
    ...
    # Calcular el porcentaje de NaN por columna en relación con el total de filas
    nan_percentage = df_muestra.isna().sum() / total_rows * 100

    # Obtener el tipo de dato de cada columna
    data_types = df_muestra.dtypes

    # Asegurarse de que las columnas numéricas son tratadas correctamente
    # Convertir las columnas que deberían ser numéricas
    df_muestra = df_muestra.apply(pd.to_numeric, errors='coerce')

    # Obtener el valor máximo y mínimo de cada columna numérica
    max_values = df_muestra.max(numeric_only=True)
    min_values = df_muestra.min(numeric_only=True)

    # Crear un DataFrame con los resultados
    nan_summary = pd.DataFrame({
        'Variable': nan_percentage.index,
        'Porcentaje de NaN': nan_percentage.values,
        'Tipo de Dato': data_types.values,
        'Valor Máximo': max_values.values,
        'Valor Mínimo': min_values.values
    })

    # Ordenar de mayor a menor porcentaje de NaN
    nan_summary_sorted = nan_summary.sort_values(by='Porcentaje de NaN', ascending=False)

    # Exportar los resultados a un archivo Excel
    nan_summary_sorted.to_excel('nan_summary_sorted_max_min.xlsx', index=False)

    # Imprimir el resumen completo de los NaN con tipo de dato, máximo y mínimo
...
def stepwise_bic(X, y, offset, family):
    ...
    while remaining:
        ...
        # Probar eliminar cada variable restante
        for var in remaining:
            X_subset = current_X.drop(columns=[var])
            model = sm.GLM(y, X_subset, family=family, offset=offset).fit()
            bic = calculate_bic(model)
            bic_scores.append((var, bic, model))
        
        # Seleccionar la mejor variable para eliminar
        var_to_remove, min_bic, best_result = min(bic_scores, key=lambda x: x[1])
        
        if min_bic < best_bic:
            best_bic = min_bic
            best_model = best_result
            current_X = current_X.drop(columns=[var_to_remove])
            remaining.remove(var_to_remove)
            print(f"Variable eliminada: {var_to_remove}, BIC: {min_bic}")
        else:
